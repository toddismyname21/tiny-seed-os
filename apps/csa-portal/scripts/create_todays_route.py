#!/usr/bin/env python3
"""Create (or fetch) today's delivery route as admin, then print the stop list
so we know exactly what the driver's Route tab will show."""
...
frag = urlparse(r2.headers.get('Location', '')).fragment
q = parse_qs(frag)
...
resp = requests.post(f'{SITE}/api/admin/route', cookies=cookies,
    headers={'User-Agent': UA, 'Content-Type': 'application/json',
             'Origin': SITE, 'Referer': SITE + '/admin/route'}, json={})
print(f'POST /api/admin/route -> {resp.status_code}')
print(resp.text[:800])
