...
// ═══════════════════════════════════════════════════════════════════════════
// RECLASSIFICATION & ML LEARNING
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Reclassify an email - human override that trains the ML system
 * @param {Object} params - threadId, newPriority, newCategory, reason
 */
function reclassifyEmail(params) {
  const { threadId, newPriority, newCategory, reason } = params;
  ...
  try {
    ...
    const oldPriority = sheet.getRange(row, priorityCol).getValue();
    const oldCategory = sheet.getRange(row, categoryCol).getValue();
    ...
    if (newCategory && ['CUSTOMER', 'VENDOR', 'INTERNAL', 'MARKETING', 'PERSONAL'].includes(newCategory)) {
      sheet.getRange(row, categoryCol).setValue(newCategory);
    }
    sheet.getRange(row, updatedCol).setValue(new Date().toISOString());

    // Log for ML learning - this is the training signal
    logChiefOfStaffAudit({
      agent: 'H', // Human override
      ...
      output: { success: true },
      humanOverride: true,
      overrideReason: reason || 'User reclassification'
    });
...
/**
 * Get full inbox state for Chief of Staff Dashboard
 * Returns all emails with their current state, sorted by priority/date
 */
function getEmailInboxState(params = {}) {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(EMAIL_INBOX_STATE_SHEET);

    if (!sheet || sheet.getLastRow() <= 1) {
      return { success: true, data: [], counts: { critical: 0, high: 0, medium: 0, low: 0 } };
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Count by priority (non-archived only)
...
/**
 * Get combined communications (email + SMS) for unified view
 */
function getCombinedCommunications(params = {}) {
  try {
    ...
    // Get emails
    const emails = getEmailInboxState({ limit, includeArchived: 'false' });
    if (emails.success && emails.data) {
      emails.data.forEach(e => {
        combined.push({
          ...
          from: e.fromname || e.from,
          subject: e.subject,
          preview: e.aisummary || e.subject,
          priority: e.priority,
          category: e.category,
          status: e.status,
          timestamp: e.receivedat,
          suggestedAction: e.aisuggestedaction
        });
      });
    }

    // Get SMS (if SMS log exists)
    try {
      const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
      ...
      if (smsSheet && smsSheet.getLastRow() > 1) {
        ...
        const fromCol = smsHeaders.indexOf('From') >= 0 ? smsHeaders.indexOf('From') : 0;
        const bodyCol = smsHeaders.indexOf('Body') >= 0 ? smsHeaders.indexOf('Body') : 2;
        const timestampCol = smsHeaders.indexOf('Timestamp') >= 0 ? smsHeaders.indexOf('Timestamp') : 4;
        const statusCol = smsHeaders.indexOf('Status') >= 0 ? smsHeaders.indexOf('Status') : -1;

        for (let i = Math.max(1, smsData.length - limit); i < smsData.length; i++) {
          const row = smsData[i];
          combined.push({
            type: 'sms',
            id: `sms_${i}`,
            ...
            subject: '',
            preview: row[bodyCol],
            priority: 'MEDIUM', // Default, can be enhanced with AI
            category: 'CUSTOMER',
            status: statusCol >= 0 ? row[statusCol] : 'NEW',
      ...
      // SMS sheet might not exist, that's OK
      Logger.log('SMS fetch error: ' + smsError.toString());
    }

    // Sort combined by timestamp (newest first)
    combined.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));

    return {
      success: true,
      data: combined.slice(0, limit),
      counts: emails.counts || { critical: 0, high: 0, medium: 0, low: 0 }
    };
  } catch (error) {
    return { success: false, error: error.toString() };
  }
...
function completeAction(params) {
  ...
  try {
    ...
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    const statusCol = headers.indexOf('Action_Status') + 1;
    const executedCol = headers.indexOf('Executed_At') + 1;
    const resultCol = headers.indexOf('Execution_Result') + 1;
    const notesCol = headers.indexOf('Notes') + 1;

    sheet.getRange(row, statusCol).setValue('COMPLETED');
    sheet.getRange(row, executedCol).setValue(new Date().toISOString());
    sheet.getRange(row, resultCol).setValue('Completed by user');
    if (notes && notesCol > 0) {
    ...
    return { success: true, data: { actionId, status: 'COMPLETED' } };
  } catch (error) {
    return { success: false, error: error.toString() };
  }
}
...
function dismissAction(params) {
  ...
  try {
    ...
    sheet.getRange(row, statusCol).setValue('REJECTED');
    sheet.getRange(row, executedCol).setValue(new Date().toISOString());
    sheet.getRange(row, resultCol).setValue('Dismissed by user');
    ...
    logChiefOfStaffAudit({
      ...
      output: { success: true },
      humanOverride: true,
      overrideReason: reason || 'User dismissed'
    });

    return { success: true, data: { actionId, status: 'REJECTED' } };
  } catch (error) {
    return { success: false, error: error.toString() };
  }
}
