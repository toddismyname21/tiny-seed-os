...
class CalendarIntegration:
    ...
    def _get_access_token(self) -> Optional[str]:
        """
        Get valid access token from OAuth manager or google_oauth.

        Tries google_oauth first (new seamless OAuth), then falls back to oauth_manager.
        ...
        # Try new google_oauth first (seamless one-click OAuth)
        try:
            from google_oauth import get_valid_access_token, load_tokens_from_supabase
            token = get_valid_access_token(self.user_id)
            if token:
                # Validate scopes from stored token data
                token_data = load_tokens_from_supabase(self.user_id)
                if token_data and token_data.get('scopes'):
                    validate_scopes(token_data['scopes'])
            ...
            print(f"[Calendar] google_oauth error: {e}")

        # Fall back to legacy oauth_manager
        try:
