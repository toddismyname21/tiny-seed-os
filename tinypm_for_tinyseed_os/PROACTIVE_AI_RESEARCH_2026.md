...
# APPENDIX A: IMPLEMENTATION CODE FOR predictive_intent.py

## A.1 Confidence Calibrator (Add to predictive_intent.py)

```python
class ConfidenceCalibrator:
    """
    Temperature scaling + historical calibration for accurate confidence scores.

    Modern neural networks are systematically overconfident. This calibrator
    ensures our confidence scores actually match the probability of being correct.

    Source: https://medium.com/data-science/confidence-calibration-for-deep-networks-why-and-how-e2cd4fe4a086
    """

    CALIBRATION_FILE = APP_DIR / ".pm_calibration.json"

    def __init__(self):
        self.temperatures = {}  # action_type -> learned temperature
        self.default_temperature = 1.5  # Conservative default (reduces overconfidence)
        self._load_state()
    ...
    def _save_state(self):
        data = {'temperatures': self.temperatures, 'updated': datetime.now().isoformat()}
        self.CALIBRATION_FILE.write_text(json.dumps(data, indent=2))

    def calibrate(self, raw_confidence: float, action_type: str,
                  historical_accuracy: Optional[float] = None) -> float:
        """
        Apply temperature scaling and historical calibration.

        Args:
            raw_confidence: The raw prediction confidence (0-1)
            action_type: The action being predicted
            historical_accuracy: Known accuracy for this action type (0-1)

        Returns:
            Calibrated confidence score (0-1)
        """
        # Step 1: Temperature scaling (reduces overconfidence)
    ...
    def update_from_outcome(self, action_type: str, predicted_conf: float, was_correct: bool):
        """
        Learn optimal temperature from prediction outcomes.

        If we predicted 80% confidence but only 60% were correct,
        we need to increase temperature to reduce future confidence.
        """
        current_temp = self.temperatures.get(action_type, self.default_temperature)

        if was_correct:
            adjustment = -0.02  # Be slightly more confident
        else:
            adjustment = 0.05  # Be less confident

        # Weighted by how confident we were (bigger errors matter more)
        adjustment *= predicted_conf

        new_temp = max(0.5, min(3.0, current_temp + adjustment))
        self.temperatures[action_type] = new_temp

        if len(self.temperatures) % 5 == 0:
            self._save_state()
```

## A.2 Task Boundary Detector
...
class TaskBoundaryDetector:
    ...
    Research shows suggestions delivered at task boundaries have
    49.7% faster response times than mid-task interruptions.

    Source: https://www.researchgate.net/publication/330831884_Interruption_Timing_Prediction
    """

    MIN_SUGGESTION_INTERVAL = 120  # 2 minutes between suggestions

    BOUNDARY_ACTIONS = {
        'complete_task', 'send_message', 'send_email', 'end_meeting',
        'archive_task', 'close_document', 'save_file'
    }

    SIGNIFICANT_SWITCHES = {
        (ActionCategory.DEEP_WORK, ActionCategory.COMMUNICATION),
        (ActionCategory.COMMUNICATION, ActionCategory.DEEP_WORK),
        (ActionCategory.CALENDAR, ActionCategory.TASK_MANAGEMENT),
    }
    ...
    def is_good_moment(self, new_action: Optional[ActionEvent] = None) -> Tuple[bool, str]:
        """Check if this is a good moment to show a suggestion."""
        now = datetime.now()

        # Respect minimum interval
        if self.last_suggestion_time:
            since_last = (now - self.last_suggestion_time).total_seconds()
            if since_last < self.MIN_SUGGESTION_INTERVAL:
                return False, "too_soon_after_last_suggestion"

        # Natural pause detection (>5 min inactivity)
        pause_seconds = (now - self.last_activity_time).total_seconds()
        if pause_seconds > 300:
            return True, "natural_pause"

        if new_action:
            if new_action.action_type in self.BOUNDARY_ACTIONS:
                return True, "task_completion"

            if self.current_category and new_action.category != self.current_category:
                switch = (self.current_category, new_action.category)
                if switch in self.SIGNIFICANT_SWITCHES:
                    return True, "context_switch"
                if self.activity_streak >= 3:
                    return True, "category_change"

        return False, "mid_task"

    def record_activity(self, action: ActionEvent):
        if action.category == self.current_category:
...
## A.3 Implicit Feedback Collector

```python
class ImplicitFeedbackCollector:
    """
    Automatically infer user feedback from behavior.

    Instead of asking "was this helpful?", we observe:
    - Did they do the suggested action? (implicit accept)
    - Did they ignore it for 30+ min? (implicit reject)
    - Did they do something else immediately? (explicit reject)
    """

    FEEDBACK_FILE = APP_DIR / ".pm_implicit_feedback.json"
    ...
    def _load_history(self) -> List[Dict]:
        if self.FEEDBACK_FILE.exists():
            try:
                return json.loads(self.FEEDBACK_FILE.read_text()).get('history', [])
            except:
                pass
        return []

    def _save_history(self):
        recent = self.feedback_history[-500:]
        data = {'history': recent, 'updated': datetime.now().isoformat()}
    ...
    def track_suggestion(self, suggestion: ProactiveSuggestion) -> str:
        ...
        self.pending_suggestions[sid] = {
            'suggestion': suggestion,
            'shown_at': datetime.now(),
            'predicted_action': suggestion.prediction.action_type,
            'confidence': suggestion.prediction.confidence
        }
        return sid

    def process_user_action(self, action: ActionEvent) -> Optional[Dict]:
        feedback = None
        expired_ids = []

        for sid, data in self.pending_suggestions.items():
            shown_at = data['shown_at']
            predicted = data['predicted_action']
            confidence = data['confidence']
            elapsed = (datetime.now() - shown_at).total_seconds()

            # User did the exact predicted action
            if action.action_type == predicted:
                feedback = {
                    'suggestion_id': sid,
            ...
            # User did something in same category (partial accept)
            elif action.category.value == data['suggestion'].prediction.category.value:
                if elapsed < 300:
                    feedback = {
                        'suggestion_id': sid,
                        'predicted_action': predicted,
    ...
    def get_acceptance_rate(self, action_type: str = None, window_days: int = 30) -> float:
        cutoff = datetime.now() - timedelta(days=window_days)
        relevant = [
            f for f in self.feedback_history
            if datetime.fromisoformat(f['timestamp']) > cutoff
            and (action_type is None or f['predicted_action'] == action_type)
        ]
        if not relevant:
            return 0.0
        accepted = len([f for f in relevant if f['outcome'] in ['accepted', 'partial_accept']])
        return accepted / len(relevant)
```

## A.4 Energy/Focus Estimator

```python
class EnergyFocusEstimator:
    """
    Estimate user's current energy and focus capacity.

    Uses circadian rhythm, recent activity, and calendar context.
    """

    PERSONAL_PATTERNS_FILE = APP_DIR / ".pm_energy_patterns.json"
    ...
    def __init__(self):
        self.personal_curve = self._load_patterns()

    def _load_patterns(self) -> Dict[int, float]:
        if self.PERSONAL_PATTERNS_FILE.exists():
            try:
                data = json.loads(self.PERSONAL_PATTERNS_FILE.read_text())
                return {int(k): v for k, v in data.get('curve', {}).items()}
            except:
                pass
        return self.DEFAULT_ENERGY_CURVE.copy()

    def estimate(self, hour: int, context: FusedContext,
                 recent_actions: List[ActionEvent]) -> Dict[str, float]:
        base_energy = self.personal_curve.get(hour, 0.5)

        if recent_actions:
            last_hour = [a for a in recent_actions
                        if (datetime.now() - a.timestamp).seconds < 3600]
            activity_level = min(len(last_hour) / 10, 1.0)
            ...
            base_energy = min(1.0, max(0.1, base_energy + energy_mod))

        focus = 0.5
        if context.free_time_minutes > 60:
            focus += 0.2
        if context.is_morning and not context.busy_day:
            focus += 0.15
        if not context.is_in_meeting and context.next_meeting_in_minutes and context.next_meeting_in_minutes > 30:
            focus += 0.15
        if context.urgent_count > 2:
            focus -= 0.2
        if context.meeting_pressure > 0.5:
            focus -= 0.2
        ...
        meeting_fatigue = min(context.meetings_today * 0.15, 0.8)
        if context.is_in_meeting:
            meeting_fatigue += 0.2

        if focus > 0.7 and base_energy > 0.6:
            optimal = "deep_work"
        elif meeting_fatigue > 0.5:
            optimal = "low_energy_tasks"
        elif base_energy > 0.7:
        ...
        return {
            ...
            'focus_capacity': round(focus, 2),
            'meeting_fatigue': round(meeting_fatigue, 2),
            'optimal_task_type': optimal
        }
```

## A.5 Integration: Updated PredictiveIntentEngine

```python
class PredictiveIntentEngine:
    """Enhanced unified interface with SOTA techniques."""
    ...
    def __init__(self):
        ...
        # NEW: Enhanced prediction components
    ...
    def predict_next_actions(self, context: FusedContext = None,
                             recent_actions: List[ActionEvent] = None,
                             limit: int = 5) -> List[PredictedAction]:
        """Enhanced prediction with calibration and energy awareness."""

        if context is None:
            context = self.context_engine.gather_context(recent_actions)

        # Get energy/focus state
        energy_state = self.energy_estimator.estimate(
            context.hour, context, recent_actions or []
        )

        # Get raw predictions
        raw_predictions = self.intent_engine.predict_next_actions(
            context, recent_actions, limit * 2
        )

        # Calibrate confidence scores
        calibrated = []
        for pred in raw_predictions:
            historical_acc = self.intent_engine._get_historical_accuracy(pred.action_type)
            new_confidence = self.confidence_calibrator.calibrate(
                pred.confidence, pred.action_type, historical_acc
            )

            # Adjust for energy/focus
            if energy_state['optimal_task_type'] == 'deep_work':
                if pred.category == ActionCategory.DEEP_WORK:
                    new_confidence *= 1.1
                elif pred.category == ActionCategory.COMMUNICATION:
                    new_confidence *= 0.9
            elif energy_state['optimal_task_type'] == 'low_energy_tasks':
                if pred.category in [ActionCategory.ADMINISTRATIVE, ActionCategory.COMMUNICATION]:
                    new_confidence *= 1.1
                elif pred.category == ActionCategory.DEEP_WORK:
                    new_confidence *= 0.8
            ...
            calibrated.append(PredictedAction(
                ...
                reasoning=pred.reasoning + [f"Energy state: {energy_state['optimal_task_type']}"],
                suggested_time=pred.suggested_time,
                supporting_evidence={
                    **pred.supporting_evidence,
                    'energy': energy_state['energy'],
                    'focus': energy_state['focus_capacity']
                },
                action_level=self._get_action_level(new_confidence)
            ))

        calibrated.sort(key=lambda p: p.confidence, reverse=True)

        self._last_context = context
        self._last_predictions = calibrated[:limit]

        return self._last_predictions

    def _get_action_level(self, confidence: float) -> str:
        if confidence >= 0.95:
            return "auto"
...
| Metric | Current Baseline | Target | SOTA Benchmark |
|--------|------------------|--------|----------------|
| Accuracy@1 | Unknown | 40% | Netflix FM-Intent: 7.4% improvement |
| Accuracy@3 | Unknown | 70% | IntentRec: Real-time updates |
| Acceptance Rate | Unknown | 52% | IUI Research: Task boundary timing |
| Calibration Error | Unknown | <10% | Temperature scaling standard |
| Response Time | N/A | <30s | - |

---

# APPENDIX C: ADDITIONAL SOURCES

## Transformer Intent Models
...
- [IntentRec (arXiv)](https://arxiv.org/html/2408.05353v1)
- [Pocket FM Sequential Models](https://xtra.pocketfm.com/out-of-pocket/modeling-sequential-user-behavior-with-transformers-a-deep-dive-into-multi)

## Confidence Calibration
- [Confidence Calibration for Deep Networks](https://medium.com/data-science/confidence-calibration-for-deep-networks-why-and-how-e2cd4fe4a086)
- [IBM Uncertainty Quantification](https://www.ibm.com/think/topics/uncertainty-quantification)
- [KDD 2025 Tutorial: LLM Calibration](https://xiao0o0o.github.io/2025KDD_tutorial/)
...
- [Design Psychology of Trust (UXmatters)](https://www.uxmatters.com/mt/archives/2025/11/the-design-psychology-of-trust-in-ai-crafting-experiences-users-believe-in.php)

## Feedback Loops
- [Feedback Loops in ML Systems](https://www.newsletter.swirlai.com/p/sai-24-feedback-loops-in-machine)
- [AI Feedback Loop (Zendesk)](https://www.zendesk.com/blog/ai-feedback-loop/)
